...
LoRa::LoRa(int miso_pin, int mosi_pin, int sck_pin, int cs_pin, int reset_pin, int dio0_pin) :
  _miso_pin(miso_pin), _mosi_pin(mosi_pin), _sck_pin(sck_pin), 
  _ss(cs_pin), _reset(reset_pin), _dio0(dio0_pin),
  _frequency(0),
  _packetIndex(0),
...
int LoRa::begin(long frequency)
{
  ...
  // start SPI
  // _spi->begin();
  spi_init(LORA_DEFAULT_SPI, LORA_DEFAULT_SPI_FREQUENCY);
  ...
  bi_decl(bi_3pins_with_func(_miso_pin, _mosi_pin, _sck_pin, GPIO_FUNC_SPI));
  gpio_init(_ss);
  gpio_set_dir(_ss, GPIO_OUT);
  gpio_put(_ss, 1);
...
size_t LoRa::write(uint8_t uint8_t)
{
...
#endif
    // attachInterrupt(digitalPinToInterrupt(_dio0), LoRa::onDio0Rise, RISING);
    gpio_set_irq_enabled_with_callback(_dio0, GPIO_IRQ_EDGE_RISE, true, &LoRa::onDio0Rise);
  } else {
...
void LoRa::onCadDone(void(*callback)(bool))
{
  _onCadDone = callback;
...
void LoRa::channelActivityDetection(void)
{
...
inline uint8_t LoRa::bitWrite(uint8_t x, uint8_t n, bool b) {
  return (x & ~(1 << n)) | (static_cast<uint8_t>(b) << n);
}
...
void LoRa::setLdoFlagForced(const bool ldoOn)
{
  uint8_t config3 = readRegister(REG_MODEM_CONFIG_3);
  bitWrite(config3, 3, ldoOn);
  writeRegister(REG_MODEM_CONFIG_3, config3);
...
void LoRa::enableLowDataRateOptimize()
{
   setLdoFlagForced(true);
}

void LoRa::disableLowDataRateOptimize()
{
...
uint8_t LoRa::singleTransfer(uint8_t address, uint8_t value)
{
  ...
  // _spi->endTransaction();
  asm volatile("nop \n nop \n nop");
  gpio_put(_ss, 0);  // Active low
  asm volatile("nop \n nop \n nop");
  spi_write_blocking(LORA_DEFAULT_SPI, &address, 1);
  spi_write_read_blocking(LORA_DEFAULT_SPI, &value, &response, 1);
  asm volatile("nop \n nop \n nop");
  gpio_put(_ss, 1);
  asm volatile("nop \n nop \n nop");
